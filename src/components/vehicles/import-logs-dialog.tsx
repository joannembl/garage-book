...
export function ImportLogsDialog({ vehicleId, isPro, onImportComplete }: ImportLogsDialogProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <Button variant="outline" size="sm" className="flex items-center gap-2 text-xs font-semibold h-9" onClick={() => setIsOpen(true)} type="button">
              ...
              <div className="border rounded-lg overflow-x-auto">
                <table className="w-full text-xs text-left">
                  <thead className="bg-muted border-b">
                    <tr>
                      {fields.filter(f => (mapping as Record<string, string>)[f.key]).map(f => (
                        <th key={f.key} className="px-3 py-2 font-bold">{(mapping as Record<string, string>)[f.key]}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      <tr key={i} className="border-b last:border-0">
                        {fields.filter(f => (mapping as Record<string, string>)[f.key]).map(f => (
                          <td key={f.key} className="px-3 py-2 text-muted-foreground truncate max-w-[150px]">
                            {String(row[(mapping as Record<string, string>)[f.key]])}
                          </td>
                        ))}
                      </tr>
                    ))}
